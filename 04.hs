...
instance Eq State where
    x == y = state_minute x == state_minute y

instance Ord State where
    x `compare` y = state_minute x `compare` state_minute y


parse_note :: [Char] -> (Note, Int, Int)
parse_note x = 
    let minute = read (take 2 (drop 15 x))
...
-- returns (Guard, State)
get_time_intervals :: [(Note, Int, Int)] -> Int -> [(Int, State)]
get_time_intervals [] _ = []
get_time_intervals ((GShift g, hour, minute):xs) _ = 
    get_time_intervals xs g

get_time_intervals ((GWakes, _, minute):xs) g =
    [(g, Awaken minute)] ++ get_time_intervals xs g

get_time_intervals ((GSleeps, _, minute):xs) g =
